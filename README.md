...
4. **设置编辑密码（重要）**：
```javascript
const EDIT_PASSWORD = 'your_custom_password';  // 修改为你自己的密码
```
...
为了保护你的甜蜜回忆，本应用支持安全验证功能：

- **只读模式**：未验证的访客只能查看内容，无法编辑
- **编辑模式**：输入正确密码后可以添加、编辑、删除内容
- **会话保持**：验证状态保存1小时，关闭浏览器后需重新验证
- **默认密码**：`love2022`（强烈建议修改！）

**如何修改密码**：
1. 编辑 `config.js` 文件
2. 修改 `EDIT_PASSWORD` 的值
3. 保存文件并刷新页面

**如何使用**：
1. 打开网页后，右上角会显示"只读模式"提示
2. 点击"登录编辑"按钮
3. 输入密码进行验证
4. 验证成功后即可编辑内容
5. 点击"退出登录"可返回只读模式
